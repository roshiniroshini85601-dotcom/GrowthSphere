...
import {
  Plus, Trash2, CheckCircle2, Loader2, ChevronDown, BookOpen, Layers, Sparkles, AlertCircle
} from 'lucide-react'
...
export default function CreateTopicPage() {
  ...
  async function handleFinish(values: WizardValues) {
    setSubmitting(true)
    try {
      ...
      toast.success('Module architecture initialized!')
  ...
  return (
    <DashboardLayout
      ...
      pageSubtitle="Add a new training topic"
    >
            ...
            >
                 ...
                 <div className="relative z-10 w-full flex flex-col md:flex-row items-center justify-between gap-12">
                    <div className="space-y-6 text-center md:text-left">
                       <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/20 border border-primary/20 backdrop-blur-md">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">System Interface</span>
                       </div>
                       <div className="space-y-2">
                       <h1 className="text-5xl lg:text-7xl font-black tracking-tighter text-white leading-none">New <span className="text-primary">Topic</span></h1>
                       <p className="text-slate-400 font-bold text-lg max-w-md leading-relaxed">Design a new training topic for your interns.</p>
                       </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <FormField control={form.control} name={`milestones.${idx}.name`} render={({ field }) => (
                            <FormItem className="md:col-span-2">
                              <FormLabel className="text-[11px] font-bold text-muted-foreground ml-1 uppercase tracking-widest">Name *</FormLabel>
              ...
              <motion.div variants={itemVars} className="flex items-center justify-between gap-6 pt-6 bg-card border-2 border-muted/50 rounded-[2.5rem] p-6 shadow-sm">
                <div className="hidden sm:flex items-center gap-3 text-muted-foreground/60 px-4">
                   <AlertCircle size={16} />
                   <p className="text-[10px] font-bold uppercase tracking-widest leading-none">Topics are shared across the team.</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <Button type="submit" disabled={submitting} className="rounded-2xl h-12 px-10 font-black shadow-xl shadow-primary/20 transition-all active:scale-95 min-w-[180px]">
                    {submitting ? (
                      ...
                      <><CheckCircle2 size={18} className="mr-2" /> Create Topic</>
                    )}
                  </Button>
                </div>
              </motion.div>
